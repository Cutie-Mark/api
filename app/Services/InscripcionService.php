...
class InscripcionService
{
    public function crearInscripciones(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            // 2) Buscar postulante existente comparando CI desencriptado
            $ciPlano = $data['ci'];
            $existingPostulante = Postulante::all(['id', 'ci', 'curso'])
            ...
            if ($existingPostulante) {
                $cursoAnterior = $existingPostulante->curso;
            }
            ...
            if (!$existingPostulante) {
                ...
                $postulante = Postulante::create([
                    'ci'                => $ciPlano, // mutator cifrará automáticamente
                ...
                // Si existía, actualizar solo campos no cifrados
                $postulante = Postulante::find($existingPostulante->id);
                if (! $postulante) {
                    throw new ModelNotFoundException('Postulante no encontrado');
                }
                $postulante->update([
                    'nombres'           => ucwords(strtolower($data['nombres'])),
                    'apellidos'         => ucwords(strtolower($data['apellidos'])),
                    ...
                    'curso'             => $data['curso'],
                    'provincia_id'      => $data['provincia'],
                    // No actualizamos 'ci' para no reescribir el cifrado existente
                ]);
                $postulanteCreado = false;
            }

            // 4) Reemplazar contactos: eliminar viejos y crear nuevos
            ...
            // 5) Obtener lista y olimpiada
            $lista = Lista::with('olimpiada')
            ...
            // 6) Si cambian de curso y ya tenían inscripciones con pagos o completas, bloquear o eliminar
            if (!$postulanteCreado && $cursoAnterior !== null && $cursoAnterior != $data['curso']) {
                ...
                if ($tienePagosOCompletas) {
                    throw new \Exception('Este postulante ya se encuentra en proceso de inscripción; no se pueden cambiar los datos de inscripción');
                }

                // Si no hay pagos pendientes ni inscripciones completas, eliminar las inscripciones existentes
            ...
            // 7) Contar inscripciones previas (si se borraron, será 0; si no, la cantidad existente)
            ...
            foreach ($data['niveles_competencia'] as $areaInput) {
                // 9.a) Verificar duplicados exactos (misma área y misma categoría)
